...
class GenerateInventoryAndManufacturingListsView(APIView):
    def post(self, request):
        ...
        for op in orderParts:
            print(op.__dict__)
        '''
        #get a list of the sku_colors from the parts in the order
        orderPartSkuColor = list(orderParts.values_list('sku_color', flat=True))
        '''
        ...
        if inventory.exists() == False or (len((set(orderPartSkuColor)) - set(list(inventory.values_list("sku_color", flat=True)))) > 0):
            ...
            if (InventoryPicklist.objects.filter(order_id=order).exists()) == False:
                ...
                manuSkus = OrderPart.objects.filter(Q(order_id = order) & ~Q(sku_color__in = inventoryPicklistSkus)).values_list('sku_color', flat=True)
